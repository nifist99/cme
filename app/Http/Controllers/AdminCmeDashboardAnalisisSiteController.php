<?php namespace App\Http\Controllers;
	...
	class AdminCmeDashboardAnalisisSiteController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    */
		...
		public function getIndex() {
			//First, Add an auth
			 ...
			 $data = [];
			 $id_site=g('parent_id');
			 $data['page_title'] = 'Analisis Site';
			 
			 $req=DB::table('cme_request')
			 ->where('status','approve')
			 ->where('id_cme_site',$id_site)->get();

			 
			$data['site']=DB::table('cme_site')
			->where('id',$id_site)
			->first();

			//ambil data id request untuk ambil data material
			$id_req=[];
			foreach($req as $key){
				$list['id_cme_request']=$key->id;
				array_push($id_req,$list);
			}
